...
FindReplaceForm::FindReplaceForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FindReplaceForm), plainTextEdit(0)
{
    ui->setupUi(this);
    ...
    // = new QLineEdit((textEdit->textCursor().selectedText()), this);
}
...
void FindReplaceForm::setPlainTextEdit(QPlainTextEdit* plainTextEdit_) {
    if (plainTextEdit) {
        // Disconnect old control:
         disconnect(plainTextEdit, 0, ui->replaceButton, 0);
         disconnect(plainTextEdit, 0, ui->replaceAllButton, 0);
    }
    plainTextEdit = plainTextEdit_;
    ui->textToFind->setText(plainTextEdit->textCursor().selectedText());
    if (!plainTextEdit) return;
     connect(plainTextEdit, SIGNAL(copyAvailable(bool)), ui->replaceButton, SLOT(setEnabled(bool)));
     connect(plainTextEdit, SIGNAL(copyAvailable(bool)), ui->replaceAllButton, SLOT(setEnabled(bool)));
}
...
void FindReplaceForm::find(bool next) {
    ...
    if (ui->regexCheckBox->isChecked()) {
        ...
        textCursor = plainTextEdit->document()->find(reg, textCursor, flags);
        if (!textCursor.isNull()) plainTextEdit->setTextCursor(textCursor);
        ...
        result = plainTextEdit->find(toSearch, flags);
    }
...
void FindReplaceForm::replace() {
    ...
    if (!plainTextEdit->textCursor().hasSelection()) {
        //plainTextEdit->textCursor().insertText(ui->textToReplace->text());
        find();
    } else {
        plainTextEdit->textCursor().insertText(ui->textToReplace->text());
        find();
...
void FindReplaceForm::replaceAll() {
    if (!plainTextEdit) return;
    int i=0;
    while (plainTextEdit->textCursor().hasSelection()){
        plainTextEdit->textCursor().insertText(ui->textToReplace->text());
        find();
        i++;
    }
